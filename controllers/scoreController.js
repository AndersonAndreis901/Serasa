...
exports.consultarScore = async (req, res) => {
  ...
  try {
    ...
    const clienteResult = await pool.query(
      "SELECT nome_razao_social, telefone FROM clientes WHERE cpf_cnpj = $1",
      [cpf_cnpj]
    );
    ...
    // regra 3: penalização por consultas acima de 5
    ...
    if (consultasDia > 5 && !passouUmDia) {
      const excesso = consultasDia - 5;
    ...
    // regra 4: último dígito (ímpar = tira, par = soma)
    ...
    } else if ([0, 2, 4, 6, 8].includes(ultimoDigito)) {
    ...
    // inadimplente, nunca logou ou tem 3+ scores < 400
    ...
    // vai registra a nova consulta
    await pool.query(
      `INSERT INTO score_consultas (usuario_id, cpf_cnpj_consultado, score, data_consulta)
       VALUES ($1, $2, $3, NOW())`,
      [usuarioId, cpf_cnpj, score]
    );
